...
root_agent = Agent(
   ...
   instruction=(
       "You are a detector of spamming. You answer in yes or no. "
       "If the input text is spam, answer 'yes'. Otherwise, answer 'no'. "
       "Spam is text that contains scammy offers, repeated links, or suspicious keywords."
   ),
   tools=[google_search],
   functions=[
       {
           "name": "is_spam",
           "description": "Detect if a given text is spam.",
           "parameters": {
               "text": {"type": "string", "description": "Text to check for spam."}
           },
           "function": is_spam
       }
   ]
